...
if (-not ($Windows -or $Linux -or $CSharp)) {
    $Windows = $true; $Linux = $true; $CSharp = $true
}
...
function Build-CSharp {
    ...
    if (-not (Test-Path "$out/BotHiderImpl.dll")) { throw "C# build produced no BotHiderImpl.dll" }
    Write-Ok "BotHiderImpl.dll built"
    return $out
}
