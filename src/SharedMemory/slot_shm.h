...
inline constexpr int kTotalSize = kOff_AvatarData + kMaxSlots * kAvatarMaxBytes;
static_assert(kOff_AvatarAppliedSid + kMaxSlots * sizeof(uint64_t) <= kOff_AvatarData, "Avatar metadata overlaps avatar data");
static_assert(kOff_AvatarData + kMaxSlots * kAvatarMaxBytes <= kTotalSize, "Shared-memory data exceeds the mapping size");
