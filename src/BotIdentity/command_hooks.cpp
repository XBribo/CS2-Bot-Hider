...
namespace cs2bh
{
    ...
    static int FindManagedSlotByPersonaName(const char *name)
    {
        ...
        for (int slot = 0; slot < PersonaPool::kMaxSlots; ++slot)
        {
            if (!Manager().IsManaged(slot))
                continue;
            if (Personas().GetSlotName(slot) == name)
                return slot;
        }
        return -1;
    }
    ...
    void HiderPlugin::Hook_DispatchConCommand_Pre(
        ConCommandRef command,
        const CCommandContext &,
        const CCommand &arguments)
    {
        if (m_bSelfDisabled || !command.IsValidRef())
        ...
        if (IsBotAddCommand(commandName))
        {
            ...
            m_AddFlippedSlots.fill(
                ManagedControllerFlagSnapshot{});
            if (m_bDisguiseEnabled && !m_bRebuilding)
            {
                FlipManagedController904(
                    false, &m_AddFlippedSlots);
            }
            RETURN_META(MRES_IGNORED);
        }
        ...
        {
            const char *target =
                arguments.ArgC() >= 2 ? arguments.Arg(1) : "";
            if (target[0] && !IsBotKickGroupTarget(target))
            {
                const int slot =
                    FindManagedSlotByPersonaName(target);
                if (slot >= 0 && engine)
                {
                    ...
                    META_CONPRINTF(
                        "[BOTHIDER] bot_kick '%s' redirected to kick "
                        "for managed slot=%d\n",
                        target,
                        slot);
        ...
        if (!IsKickCommand(commandName) || m_bRebuilding)
            RETURN_META(MRES_IGNORED);

        m_ManagedBeforeKick = 0;
        m_QuotaBeforeKick = -1;
        m_AdjustQuotaAfterKick =
            std::strcmp(commandName, "bot_kick") != 0;
        if (m_AdjustQuotaAfterKick)
        {
            for (int slot = 0;
                 slot < PersonaPool::kMaxSlots;
                 ++slot)
            {
                if (Manager().IsManaged(slot))
                    ++m_ManagedBeforeKick;
        ...
        for (int slot = 0;
             ...
             ++slot)
        {
            if (!Personas().IsSlotManaged(slot))
                continue;
            void *client =
                entity_access::ResolveClientBySlot(slot);
            if (!client)
                continue;
        ...
        META_CONPRINTF(
            "[BOTHIDER] kick PRE '%s' restored=%d\n",
            commandName,
            restored);
        RETURN_META(MRES_IGNORED);
    }
    ...
    void HiderPlugin::Hook_DispatchConCommand_Post(
        ConCommandRef command,
        const CCommandContext &,
        const CCommand & /*arguments*/)
    {
        ...
        if (m_bRebuilding)
        {
            m_bRebuilding = false;
            META_CONPRINTF(
                "[BOTHIDER] disguise-off kick done\n");
            RETURN_META(MRES_IGNORED);
        }
        ...
        {
            const int removedManaged =
                m_ManagedBeforeKick - managedAfterKick;
            if (removedManaged > 0)
            {
                ConVarRefAbstract botQuota("bot_quota");
                if (botQuota.IsValidRef())
                {
                    int desiredQuota =
                        m_QuotaBeforeKick - removedManaged;
                    if (desiredQuota < 0)
                        desiredQuota = 0;
                    if (botQuota.GetInt() != desiredQuota)
                        botQuota.SetInt(desiredQuota);
                }
            }
        }
        ...
        META_CONPRINTF(
            "[BOTHIDER] kick POST '%s' redisguised=%d quota=%d\n",
            commandName,
            redisguised,
            managedAfterKick);
        RETURN_META(MRES_IGNORED);
    ...
    void HiderPlugin::SetDisguiseEnabled(bool enabled)
    {
        ...
        if (engine && managed > 0)
        {
            ...
            m_bRebuilding = true;
            const int quota =
                identity_runtime::CountHumanClients() + managed;
            ...
            META_CONPRINTF(
                "[BOTHIDER] disguise %s rebuilding %d bot(s), "
                "quota=%d\n",
                enabled ? "ON" : "OFF",
                managed,
                quota);
        ...
        META_CONPRINTF(
            "[BOTHIDER] disguise %s (no rebuild)\n",
            enabled ? "ON" : "OFF");
    }
    ...
    {
        if (m_bSelfDisabled || !m_bDisguiseEnabled ||
            !engine || m_bRebuilding)
        {
            return;
        }
        ...
        engine->ServerCommand(quotaCommand);
        META_CONPRINTF(
            "[BOTHIDER] rematch rebuild kicked %d bot(s), "
            "bot_quota->%d\n",
            managed,
            quota);
    }
    ...
    HiderPlugin::Hook_StartChangeLevel_Pre(
        const char *mapName,
        const char *landmark,
        void * /*changelevelState*/)
    {
        ...
        META_CONPRINTF(
            "[BOTHIDER] StartChangeLevel PRE map='%s' landmark='%s'\n",
            mapName ? mapName : "?",
            landmark ? landmark : "");
        RETURN_META_VALUE(MRES_IGNORED, nullptr);
    }
    ...
    void HiderPlugin::Hook_GameFrame_Post(
        bool simulating,
        bool /*bFirst*/,
        bool /*bLast*/)
    {
             ...
             ++slot)
        {
            ...
            if (client)
            {
                identity_runtime::ReleaseManagedHltvSlot(
                    slot, client);
            }
        }
        Manager().OnTick();
        ...
        if ((++m_TickCounter & 63u) == 0u)
        {
                 ...
                 ++slot)
            {
                ...
                void *client =
                    entity_access::ResolveClientBySlot(slot);
                if (client)
                    entity_access::ResetIdleTimerForClient(client);
            }
        }
