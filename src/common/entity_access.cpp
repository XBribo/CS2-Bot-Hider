...
namespace cs2bh::entity_access
{
    ...
    {
        if (!g_pNetworkServerService || slot < 0 || slot >= 64)
            return false;
        auto *gameServer = g_pNetworkServerService->GetIGameServer();
        if (!gameServer)
            return false;
        ...
        const sig::ModuleInfo &serverModule)
    {
        ...
        if (signature.empty() ||
            !sig::ParseSigString(signature, bytes, wildcards))
        {
            META_CONPRINTF(
                "[BOTHIDER] warning: UTIL_Remove %s sig missing/malformed in gamedata.json\n",
                sig::PlatformName());
            return;
        }

        auto *hit = static_cast<unsigned char *>(
            sig::FindPatternIn(serverModule, bytes, wildcards));
        if (!hit)
            return;
        ...
        {
            const bool isWindowsMov =
                bytes[i] == 0x48 && bytes[i + 1] == 0x8B &&
                bytes[i + 2] == 0x0D;
            const bool isLinuxLea =
                bytes[i] == 0x48 && bytes[i + 1] == 0x8D &&
                bytes[i + 2] == 0x05;
            ...
            const int32_t displacement =
                *reinterpret_cast<int32_t *>(displacementAddress);
            unsigned char *instructionEnd = displacementAddress + 4;
            g_ppEntSysGlobal =
                reinterpret_cast<void **>(instructionEnd + displacement);
            break;
        }
    }
    ...
    void LoadMemberOffsets(const nlohmann::json &gamedata)
    {
        ...
        ssc::OFFSET_m_UserIDString = FindPlatformOffset(
            gamedata, "CServerSideClient::m_UserIDString",
            ssc::OFFSET_m_UserIDString);
        ssc::OFFSET_m_Name = FindPlatformOffset(
            gamedata, "CServerSideClient::m_Name", ssc::OFFSET_m_Name);
        ssc::OFFSET_m_nClientSlot = FindPlatformOffset(
            gamedata, "CServerSideClient::m_nClientSlot",
            ssc::OFFSET_m_nClientSlot);
        ssc::OFFSET_m_nEntityIndex = FindPlatformOffset(
            gamedata, "CServerSideClient::m_nEntityIndex",
            ssc::OFFSET_m_nEntityIndex);
        ssc::OFFSET_m_Server = FindPlatformOffset(
            gamedata, "CServerSideClient::m_Server", ssc::OFFSET_m_Server);
        ssc::OFFSET_m_NetChannel = FindPlatformOffset(
            gamedata, "CServerSideClient::m_NetChannel",
            ssc::OFFSET_m_NetChannel);
        ssc::OFFSET_m_nConnectionTypeFlags = FindPlatformOffset(
            gamedata, "CServerSideClient::m_nConnectionTypeFlags",
            ssc::OFFSET_m_nConnectionTypeFlags);
        ssc::OFFSET_m_nSignonState = FindPlatformOffset(
            gamedata, "CServerSideClient::m_nSignonState",
            ssc::OFFSET_m_nSignonState);
        ssc::OFFSET_m_pAttachedTo = FindPlatformOffset(
            gamedata, "CServerSideClient::m_pAttachedTo",
            ssc::OFFSET_m_pAttachedTo);
        ssc::OFFSET_m_bFakePlayer = FindPlatformOffset(
            gamedata, "CServerSideClient::m_bFakePlayer",
            ssc::OFFSET_m_bFakePlayer);
        ssc::OFFSET_m_UserID = FindPlatformOffset(
            gamedata, "CServerSideClient::m_UserID", ssc::OFFSET_m_UserID);
        ssc::OFFSET_m_SteamID = FindPlatformOffset(
            gamedata, "CServerSideClient::m_SteamID", ssc::OFFSET_m_SteamID);
        ssc::OFFSET_m_SteamIDMirror = FindPlatformOffset(
            gamedata, "CServerSideClient::m_SteamIDMirror",
            ssc::OFFSET_m_SteamIDMirror);
        ssc::OFFSET_m_bIsHLTV = FindPlatformOffset(
            gamedata, "CServerSideClient::m_bIsHLTV",
            ssc::OFFSET_m_bIsHLTV);

        targets::kClientListOffset = FindPlatformOffset(
            gamedata, "CNetworkGameServerBase::m_Clients",
            targets::kClientListOffset);
        targets::kController_FakeClientFlagsOffset = FindPlatformOffset(
            gamedata, "CBasePlayerController::FakeClientFlags",
            targets::kController_FakeClientFlagsOffset);
        targets::kController_TeamOffset = FindPlatformOffset(
            gamedata, "CBaseEntity::m_iTeamNum",
            targets::kController_TeamOffset);
        targets::kVTSlot_ClientSetName = FindPlatformOffset(
            gamedata, "CServerSideClient::SetName",
        ...
        size_t capacity)
    {
        ...
        {
            const char *source =
                *reinterpret_cast<const char *const *>(address);
            if (!source)
            {
                output[0] = '\0';
                return false;
        ...
        size_t classnameCap)
    {
        if (classnameOut && classnameCap)
            classnameOut[0] = '\0';
        if (!g_pGameResourceService || entityIndex <= 0 ||
            entityIndex >= 0x8000 ||
        ...
        void *chunk = nullptr;
        const void *chunkSlot =
            reinterpret_cast<unsigned char *>(entitySystem) +
            targets::kEntSys_IdentityChunksOffset +
            (entityIndex / targets::kEntListChunkSize) * sizeof(void *);
        if (!SafeReadPointer(chunkSlot, &chunk) || !chunk)
            return nullptr;

        unsigned char *identity =
            reinterpret_cast<unsigned char *>(chunk) +
            (entityIndex % targets::kEntListChunkSize) *
                targets::kEntIdentity_Size;
        if (classnameOut && classnameCap)
        {
            SafeReadString(
                identity + targets::kEntIdentity_ClassNameOffset,
                classnameOut,
                classnameCap);
        }
        ...
        if (!SafeReadPointer(
                identity + targets::kEntIdentity_InstanceOffset,
                &instance) ||
            !instance)
        {
            return nullptr;
    ...
    bool IsEntityBeingDeleted(void *instance)
    {
        ...
        const uint32_t flags =
            static_cast<uint32_t>(entity->m_pEntity->m_flags);
        return (flags &
    ...
    {
        if (g_EntitySystemCrossChecked || !g_ppEntSysGlobal ||
    ...
    void ResetIdleTimerForClient(void *client)
    {
        ...
        const int pawnOffset =
            schema::GetFieldOffset("CBasePlayerController", "m_hPawn");
        const int idleOffset = schema::GetFieldOffset(
            "CCSPlayerPawnBase", "m_flIdleTimeSinceLastAction");
        if (pawnOffset < 0 || idleOffset < 0)
            return;
        ...
        char className[64];
        void *controller =
            ResolveEntityInstance(entityIndex, className, sizeof(className));
        if (!controller ||
        ...
        const uint32_t pawnHandle = *reinterpret_cast<uint32_t *>(
            reinterpret_cast<unsigned char *>(controller) + pawnOffset);
