...
public sealed unsafe partial class NativePresentationClient
{
    ...
    [DllImport("BotHider", CallingConvention = CallingConvention.Cdecl)]
    private static extern int BotHider_GetOptions(ulong session);
    [DllImport("BotHider", CallingConvention = CallingConvention.Cdecl)]
    ...
    private int Options => Session is var session && session != 0 ? BotHider_GetOptions(session) : -1;
    internal bool AutoRespawn => Options is var options && options >= 0 && (options & 1) != 0;
    internal bool ExternalAvatars => Options is var options && (options < 0 || (options & 2) != 0);
    ...
    internal bool SetAvatar(int slot, byte[] png)
        => !ExternalAvatars && TryGetSlot(slot, out var s) && s.Managed != 0 &&
