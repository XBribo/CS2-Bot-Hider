...
internal sealed class BotHiderPresentationService : IDisposable
{
    ...
    private void PublishSlot(int slot, NativePresentationClient.Slot native, CCSPlayerController player)
    {
        ...
        try
        {
            ...
            // Ordinary servers own a base flair too; reconcile it through the
            // same change-only publisher as lease overrides and restoration.
            {
    ...
    private void ReportPresentationFailure(int slot, string reason)
    {
        ...
        if (now >= _slots[slot].NextFailureLogUtc)
        {
            ...
            Server.PrintToConsole(
                $"[BotHider] presentation write failed slot={slot}: {reason}{suffix}");
