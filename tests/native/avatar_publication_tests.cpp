...
int main()
{
    ...
    CHECK(p.entries.at(7).restore == original); // Do not restore an intermediate image.
    ...
    CHECK(!p.Prepare(7, b, original, 44)); // Another source cannot steal direct ownership.
    p.Retire(7, {});
    CHECK(p.Observe(7, {}, 2)); // Clearing data also invalidates the HUD.
    CHECK(!p.Observe(7, {}, 2));
    CHECK(p.Prepare(7, a, original, 44));
    CHECK(!p.Prepare(7, b, original, 45)); // Reused slot has a different owner.
    CHECK(p.entries.at(7).owner == 44);
