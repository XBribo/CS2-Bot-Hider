...
int main()
{
    ...
    a.PublishRelease(3);
    a.PublishAdopt(3, 222, "replacement", "", 0);
    ...
    a.UpdatePing(3, 42);
    a.UpdatePing(3, 42);
    a.UpdateSyntheticSid(3, 333);
    a.UpdatePersonaName(3, "published");
    if (pingChanges != 1 || rosterChanges != 3 || lastChangedSlot != 3) return 10;
    bool foreignAccepted = true;
    std::thread worker([&] {
        foreignAccepted = a.Matches(3, current.Session, current.Incarnation);
    });
    worker.join();
    if (foreignAccepted) return 5;
    a.Shutdown();
    if (a.Session() || a.ReadSlot(3, current)) return 6;
    a.Shutdown();
    if (rosterChanges != 4 || lastChangedSlot != -1) return 11;
    a.Init();
    if (a.Session() == old.Session || a.Matches(3, old.Session, old.Incarnation)) return 7;
